---
title: "Predicting the Number of Respondents from each State in the 2022 American Community Survey"
author: "Richard Guo"
date: today
format: pdf
editor: visual
...
doctoral_count <- doctoral_count %>%
  mutate(estimated_total = round(doctoral / california_ratio))
kable(head(doctoral_count))  %>%
  kable_styling(full_width = FALSE, font_size = 10) %>%
...
ggplot(data_melted, aes(x = factor(data_melted$STATEICP), y = value, fill = variable)) +
  ...
  labs(x = "State Code", y = "Number of Respondents", fill = "Respondent Type")   + theme_minimal() + coord_flip() + 
  scale_fill_hue(labels = c("Estimated", "Actual"))
```
